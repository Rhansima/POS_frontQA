...
public class Customer extends BaseTest {
    ...
    @Test
    public void customerDashboard() throws InterruptedException {
        loginAsCashierUser();

        WebElement customerMenuClick = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//*[@id=\"root\"]/div/div[1]/div/div[2]/nav/a[3]")
        ));
        customerMenuClick.click();

        WebElement dashboardHeader = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//h1[text()='Customer History']")
        ));
